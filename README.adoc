...
* *Enhanced security*: prepare for a future where `final` truly means `final`, enhancing program safety and potential performance. The release also continues the work on post-quantum cryptography with a second preview of a simplified PEM encoding API.
...
private final LazyConstant<MapData> mapData =
    LazyConstant.of(this::loadMapData);

public Locale getSatelliteLayer() {
  return mapData.get().getSatelliteLayer(); // <= The lazy constant is accessed here
}
----
...
Two important changes are the increased usage of auto-vectorization on supporting x64 and AArch64 architectures, and now leveraging the foreign function and memory API to link native mathematical-function libraries.
...
In this release, we see the team working on making sure we can better secure our applications against future, quantum-based computing threats.
