...
:description: A comprehensive overview of Java 26, covering new JEPs including HTTP/3 support, G1 GC improvements, structured concurrency, primitive types in patterns, ahead-of-time object caching, and security enhancements.
:keywords: java 26, jdk 26, http/3, structured concurrency, vector api, g1 gc, lazy constants, primitive patterns, jep, openjdk
...
UUIDv7's time-ordered nature enables natural chronological sorting:
...
UUIDv7 *complements* the existing v4 format rather than replacing it. The choice between them depends on your requirements:

* Use *v4* if: simplicity, maximum randomness, and broad compatibility are required, and insertion order or time ordering is unimportant.
...
====
UUIDv7 is fully compatible with existing UUID infrastructure. All standard UUID methods (`toString()`, `fromString()`, etc.) work seamlessly, and UUIDv7 can be stored in existing UUID database columns without schema changes.
====
...
This makes UUIDv7 particularly valuable for:

* Logging systems with high volume writes
* Event sourcing applications
* Time-series data storage
* Distributed systems requiring ordered identifiers
* Applications where B-tree index efficiency is critical
====
...
=== https://bugs.openjdk.org/browse/JDK-8354548[JDK-8354548 - Update CLDR to Version 48.0]

This updates the Unicode Common Locale Data Repository (CLDR) in the JDK to https://cldr.unicode.org/downloads/cldr-48[CLDR version 48].
...
=== https://bugs.openjdk.org/browse/JDK-8357653[JDK-8357653 - Inner classes of type parameters emitted as raw types in signatures]
...
=== https://bugs.openjdk.org/browse/JDK-8362637[JDK-8362637 - Convert java.nio.ByteOrder to an enum]
...
=== https://bugs.openjdk.org/browse/JDK-8364361[JDK-8364361 - java.lang.Process should implement Closeable]
...
=== https://bugs.openjdk.org/browse/JDK-8366575[JDK-8366575 - Remove SDP support]
...
Whilst this release encompasses ten JEPs, it's worth noting that it includes https://openjdk.org/jeps/517[JEP-517: HTTP/3 for the HTTP Client API], which represents one of the most substantial pull requests in recent years. Additionally, the first https://jdk.java.net/valhalla/[project Valhalla] early-access build has been made available for experimentation.
...
This release also resolves a notable issue where inner classes of type parameters were being emitted as raw types (https://bugs.openjdk.org/browse/JDK-8357653[JDK-8357653]).

Furthermore, two new https://dev.java/learn/jvm/jfr/[Java Flight Recorder] events have been introduced to provide deeper insights into application behaviour:
...
[width="100%", cols="1", frame=top, grid=none]
|===
