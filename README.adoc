...
This preview API, previously known as https://openjdk.org/jeps/502[JEP-502: Stable values], introduces immutable value holders that are at most initialised once, as it will help us move towards deferred immutability through `LazyConstant` and `StableSupplier`.
...
. The API has been simplified by removing the `function` and `intFunction` factory methods, given that they provided minimal benefits over lazy lists and maps.
. Null is no longer allowed as a computed value to improve performance, and this better aligns lazy constants with other constructs in the language, such as unmodifiable collections and `ScopedValue`
...
Two important changes are the increased usage of auto-vectorisation on
...
UUID uuid = UUID.ofEpochMillis(currentTimeMillis);
...
UUID another = UUID.ofEpochMillis(currentTimeMillis);
...
====
UUIDv7 is fully compatible with the existing UUID infrastructure. All standard UUID methods (`toString()`, `fromString()`, etc.) work seamlessly, and UUIDv7 can be stored in existing UUID database columns without schema changes.
====
